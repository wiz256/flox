...
This is my current C++ Trader 10 project code for you to understand what I'm doing and analyze if any issues or improvements hould be done, it's super fast, so maybe I have some bugs there:
...
enum StrategyKind : int
{
    ...
    SMA_CROSSOVER,  // Benchmark for engine validation
    NUM_STRATEGIES
};
...
inline const char* strategyExitMode(StrategyKind k)
{
    static const char* modes[] = {
        ...
        "death_cross"          // SMA_CROSSOVER: exit on death cross (benchmark)
    };
    return modes[k];
}
...
struct CombinedGrid
{
    ...
    CombinedGrid()
    {
        ...
        // 14. SMA Crossover (benchmark): fast=[10,20,50], slow=[50,100,200]
        //     Canonical benchmark: SMA 50/200 golden cross on BTC should show Sharpe 0.8-1.3
        for (int f : {10, 20, 50})
            for (int s : {50, 100, 200})
                if (f < s)
                    entries.push_back({SMA_CROSSOVER, {f, s}});
    }
...
static const StrategyAxisDefs g_axisDefs[NUM_STRATEGIES] = {
    ...
    /* SMA_CROSSOVER */ {
        { AxisDef{AxisDef::INT, 0,-1, {10,20,50}, {}},
          AxisDef{AxisDef::INT, 1,-1, {50,100,200}, {}} }
    },
};
...
static double g_annualizationFactor = 252.0;  // bars per year (set from timeframe)
...
Strategy* createStrategy(const CombinedParams& p)
{
    ...
    case SMA_CROSSOVER:
        return new SmaCrossoverStrategy(1, g_symbolId, g_registry,
            SmaCrossoverParams{p.ip[0], p.ip[1]});
    default:
        return new EmaCrossoverStrategy(1, g_symbolId, g_registry);
    }
}
...
BacktestResult runBacktest(const CombinedParams& p)
{
    ...
    config.metricsAnnualizationFactor = g_annualizationFactor;
...
int main(int argc, char* argv[])
{
    ...
    // Compute annualization factor from timeframe (bars per year)
    // 4h = 6/day × 365 = 2190, 1h = 24/day × 365 = 8760, 1d = 252 (trading days)
    {
        double barsPerDay = 1.0;
        if (timeframe == "4h") barsPerDay = 6.0;
        else if (timeframe == "1h") barsPerDay = 24.0;
        else if (timeframe == "15m") barsPerDay = 96.0;
        else if (timeframe == "5m") barsPerDay = 288.0;
        else if (timeframe == "1d") barsPerDay = 1.0;
        // For crypto (24/7 markets), use 365 days
        g_annualizationFactor = barsPerDay * 365.0;
    }
    if (g_annualizationFactor != 252.0)
        std::cerr << "  Annualization: " << g_annualizationFactor << " bars/year (" << timeframe << ")\n";
